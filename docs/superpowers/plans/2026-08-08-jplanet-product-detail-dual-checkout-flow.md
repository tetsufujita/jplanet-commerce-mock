...
- Fix Round 1 mobile recommendation hardening: 390px rendered exactly two fully visible cards with visible-card ratio `2.115` and third-card peek `6.41px`; 320px rendered exactly two with ratio `2.118` and peek `3.44px`. The audit requires two cards, ratio `2.05–2.20`, positive peek over `1px`, and peek below `15%` of a card, rejecting one-card, three-card, and no-peek regressions.
- Fix Round 1 order-flow styling: resolved tokens were sakura `rgb(254, 162, 172)`, navy `rgb(31, 56, 100)`, surface `rgb(255, 255, 255)`, blue-soft `rgb(243, 246, 251)`, line `rgb(229, 234, 241)`, and muted `rgb(102, 112, 133)`. Current background/border matched sakura with a surface icon; complete matched surface/navy/navy; pending matched blue-soft/line/muted. The details link computed `textDecorationLine=underline` and `textDecorationColor=rgb(254, 162, 172)` (sakura).
