...
// 出力: design/reproductions/stripe-jp/recordings/{rec-scroll,rec-hover}.webm
//       design/reproductions/stripe-jp/{stripe-jp-dom.html,fullpage.png,hover-inventory.json}
import { chromium } from '@playwright/test'
import { mkdirSync, writeFileSync } from 'node:fs'

const OUT = 'design/reproductions/stripe-jp/clone-rec'
...
// ── 1. スクロール録画（ページ全体のモーション）─────────────────
{
  ...
  writeFileSync(`${OUT}/stripe-jp-dom.html`, html)
  ...
  await context.close()
  await video.saveAs(`${REC}/rec-scroll.webm`)
  console.log('rec-scroll.webm saved')
}
...
// ── 2. hover / マウス操作録画（megamenu・ボタン・カード）────────
{
  ...
  writeFileSync(
    `${OUT}/hover-inventory.json`,
    JSON.stringify({ navItems, hoverTargets }, null, 2),
  )

  const video = page.video()
  await context.close()
  await video.saveAs(`${REC}/rec-hover.webm`)
  console.log('rec-hover.webm saved')
}
