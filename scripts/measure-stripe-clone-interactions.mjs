...
// 出力: specs/02-clone-interactions.json + specs/megamenu-dom-clone/panel-*.html
...
mkdirSync(`${OUT}/specs/megamenu-dom-clone`, { recursive: true })
const sleep = (ms) => new Promise((r) => setTimeout(r, ms))
const result = {}

const browser = await chromium.launch()
const page = await browser.newPage({ viewport: { width: 1440, height: 900 } })
await page.goto('http://localhost:5180/stripe-jp', { waitUntil: 'networkidle', timeout: 60000 })
await sleep(2500)
...
await page.evaluate(() => {
  window.__S = []
  window.__stop = false
  const header = document.querySelector('.sj-header') || document.querySelector('header')
  const chevron = document.querySelector('.sj-nav-item__chevron')
  const trigger0 = document.querySelector('.sj-nav-item')
  const loop = () => {
    if (window.__stop) return
    const panel = [...document.querySelectorAll('.sj-panel, .sj-header div')]
...
const triggers = await page.locator('.sj-nav-item').all()
...
for (let i = 0; i < Math.min(triggers.length, 4); i++) {
  ...
  const html = await page.evaluate(() => {
    const panel = document.querySelector('.sj-panel')
...
panelHtmls.forEach((h, i) => { if (h) writeFileSync(`${OUT}/specs/megamenu-dom-clone/panel-${i}.html`, h) })
result.panelsSaved = panelHtmls.filter(Boolean).length

// ───────────────────── P12: GDP ticker（12 秒監視）─────────────────────
result.ticker = await page.evaluate(async () => {
  const el = document.querySelector('.sj-hero-eyebrow-value')
...
result.marquee = await page.evaluate(async () => {
  const el = document.querySelector('.sj-marquee')
  if (!el) return { error: 'not found' }
...
// ───────────────────── P4: header sticky 挙動 ─────────────────────
{
  const readHeader = () => page.evaluate(() => {
    const h = document.querySelector('.sj-header') || document.querySelector('header')
...
{
  await page.evaluate(() => {
    const bento = document.querySelector('.sj-bento-grid')
    if (bento) bento.scrollIntoView({ block: 'center' })
  })
  await sleep(900)
  result.bento = await page.evaluate(() => {
    const cards = [...document.querySelectorAll('.sj-bento')].filter((el) => !el.className.toString().includes('--hover'))
    return { cardCount: cards.length, classes: cards.slice(0, 8).map((c) => c.className.toString().slice(0, 90)) }
  })
  const card = page.locator('.sj-bento:not([class*="--hover"])').first()
  if (await card.count()) {
...
writeFileSync(`${OUT}/specs/02-clone-interactions.json`, JSON.stringify(result, null, 2))
